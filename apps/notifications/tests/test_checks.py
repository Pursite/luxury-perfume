...
class SmsSettingsChecksTests(SimpleTestCase):
    ...
    @override_settings(
        ...
        ORDER_PROCESSING_ALERT_PHONE="not-a-phone",
    ...
    def test_enabled_sms_fails_closed_for_missing_provider_and_invalid_owner_phone(self):
        errors = check_sms_settings(None)

        assert {error.id for error in errors} >= {"notifications.E001", "notifications.E002"}

    @override_settings(
        SMS_ENABLED=True,
        SMS_PROVIDER="check-fake",
        ORDER_PROCESSING_ALERT_PHONE="09123456789",
        SMS_CONNECT_TIMEOUT_SECONDS=3,
        SMS_READ_TIMEOUT_SECONDS=7,
        SMS_OPERATION_LEASE_SECONDS=30,
        SMS_MAX_ATTEMPTS=5,
