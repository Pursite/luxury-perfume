...
def create_waiting_order(*, user: CustomUser, address_id, idempotency_key, shipping_amount=Decimal("0.00")):
        ...
        with transaction.atomic():
            ...
            if waiting is not None:
                if now < waiting.reservation_expires_at:
                    raise ActiveCheckoutError("An active checkout already exists.")
                ...
                if address is None:
                    raise CheckoutAddressError("The address does not belong to the user.")
                ...
                if len(cart_item_ids) > MAX_CART_ITEMS_FOR_CHECKOUT:
                    raise CheckoutError("The cart contains too many distinct products.")
                ...
                if len(cart_items) != len(cart_item_ids):
                    raise EmptyCartError("The cart changed during checkout.")
                subtotal = Decimal("0.00")
                for cart_item in cart_items:
                    product = products.get(cart_item.product_id)
                    if product is None or not product.is_active:
                        raise ProductUnavailableError("A cart product is unavailable.")
                    if product.stock < cart_item.quantity:
                        raise InsufficientStockError("A cart product no longer has enough stock.")
